...
function CatalogContent({
  ...
  const handleCategoryChange = (cat: string) => {
    ...
    if (slug) {
      router.push(`/services-and-products/catalog?category=${slug}`, { scroll: false });
    } else {
      router.push('/services-and-products/catalog', { scroll: false });
    }
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center pt-20">
        <div className="animate-pulse text-gray-400 font-medium">Loading catalog...</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#f8fafc]">
      ...
      <div className="gradient-hero text-white pt-32 pb-14 md:pt-40 md:pb-16 relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <h1 className="text-3xl md:text-4xl font-black mb-3 tracking-tight">Services & Products Catalog</h1>
          <p className="text-green-100/70 font-light">Explore our full range of agricultural materials and solutions</p>
        </div>
      </div>
...
export default function ServicesCatalogPage(props: {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>;
}) {
  return (
    <Suspense fallback={
